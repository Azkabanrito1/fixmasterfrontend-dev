...
import { getEstateCustomerListings } from "../../../redux/franchisee/actions";
import AddEstateCustomer from "../../../components/franchiseecomponents/estatecustomers/AddEstateCustomer";
...
const EstateListing = () => {
  ...
  useEffect(() => {
    if (!isAddEstateModalOpen) getListings();
  }, [dateQueries, isAddEstateModalOpen]);

  const getListings = async () => {
    const response = await dispatch(
      getEstateCustomerListings(dateQueries.fromDate, dateQueries.toDate)
    );
  ...
  return (
    <>
      <div>
        <PageHeading>Estate Listing</PageHeading>
      </div>
      ...
      {isAddEstateModalOpen && (
        <AddEstateCustomer
          isOpen={isAddEstateModalOpen}
          closeModal={() => setIsAddEstateModalOpen(false)}
        />
      )}
    </>
  );
