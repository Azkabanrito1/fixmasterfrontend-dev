...
const SuspendedFranchisees = () => {
  ...
  const { data: suspendedUsers, isLoading } =
    useGetSuspendedUsers("Franchisee");
        ...
        customBodyRender: (value, tableMeta) => {
          ...
          return (
            <div className="text-center">
              <GlobalTableActions actions={actions} id="unsuspend-franchisee" />
            </div>
          );
        },
      },
  ...
  return (
    <div>
      ...
      {!isLoading && suspendedUsers?.length === 0 && (
        <p className="text-center">No Suspended Franchisees</p>
      )}
    </div>
  );
};

export default SuspendedFranchisees;
