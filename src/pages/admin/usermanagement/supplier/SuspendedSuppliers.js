...
const SuspendedSuppliers = () => {
  ...
  const { data: suspendedUsers, isLoading } = useGetSuspendedUsers("Supplier");
        ...
        customBodyRender: (value, tableMeta) => {
          ...
          return (
            <div className="text-center">
              <GlobalTableActions actions={actions} id="unsuspend-supplier" />
            </div>
          );
        },
      },
  ...
  return (
    <div>
      ...
      {!isLoading && suspendedUsers?.length === 0 && (
        <p className="text-center">No Suspended Suppliers</p>
      )}
    </div>
  );
};

export default SuspendedSuppliers;
