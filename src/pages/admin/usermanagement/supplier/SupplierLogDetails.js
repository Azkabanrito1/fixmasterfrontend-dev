...
const SupplierLogDetails = () => {
  const location = useLocation();

  const stageUrl = location.pathname.split("/").slice(-1)[0];

  const { data: userLogSession, isLoading } = useGetUserLogSession("supplier");
  ...
  return (
    <div>
      ...
      {!isLoading && userLogSession?.data?.length === 0 && (
        <p className="text-center">No login details for suppliers</p>
      )}
    </div>
  );
};

export default SupplierLogDetails;
