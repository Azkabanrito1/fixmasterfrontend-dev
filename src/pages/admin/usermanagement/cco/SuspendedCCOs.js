...
const SuspendedCCOs = () => {
  ...
  const { data: suspendedUsers, isLoading } = useGetSuspendedUsers("CCO");
        ...
        customBodyRender: (value, tableMeta) => {
          ...
          return (
            <div className="text-center">
              <GlobalTableActions actions={actions} id="unsuspend-cco" />
            </div>
          );
        },
      },
  ...
  return (
    <div>
      <BackBtn />
      <PageHeading style={{ marginBottom: "32px" }}>Suspended CCOs</PageHeading>
      ...
      {!isLoading && suspendedUsers?.length === 0 && (
        <p className="text-center">No Suspended CCOs</p>
      )}
    </div>
  );
};

export default SuspendedCCOs;
