...
const QaMasterLogDetails = () => {
  const location = useLocation();

  const stageUrl = location.pathname.split("/").slice(-1)[0];

  const { data: userLogSession, isLoading } = useGetUserLogSession("qa");
  ...
  return (
    <div>
      ...
      {!isLoading && userLogSession?.data?.length === 0 && (
        <p className="text-center">No login details for QA Masters</p>
      )}
    </div>
  );
};

export default QaMasterLogDetails;
