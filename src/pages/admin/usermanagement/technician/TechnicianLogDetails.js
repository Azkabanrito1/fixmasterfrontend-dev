...
const TechnicianLogDetails = () => {
  const location = useLocation();

  const stageUrl = location.pathname.split("/").slice(-1)[0];

  const { data: userLogSession, isLoading } =
    useGetUserLogSession("technician");
  ...
  return (
    <div>
      ...
      {!isLoading && userLogSession?.data?.length === 0 && (
        <p className="text-center">No login details for technicians</p>
      )}
    </div>
  );
};

export default TechnicianLogDetails;
