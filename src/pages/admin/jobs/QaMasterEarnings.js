...
const QaMasterEarnings = ({ collaborator }) => {
  ...
  const collabRole = collaborator?.slice(0, 2)?.toLowerCase();

  const userRole = roles?.data.find(
    (roles) => roles.name?.toLowerCase() === collabRole
  );
...
export default QaMasterEarnings;
