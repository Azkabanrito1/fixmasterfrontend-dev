...
const UpcomingEvents = ({ events = [], title }) => {
  const [date, setDate] = useState(new Date());

  const eventDates = events?.map(
    (event) => new Date(event?.requestFixDate?.slice(0, 10))
  );
  let modEvents = events?.map((event) => {
    return {
      date: new Date(event?.requestFixDate?.slice(0, 10)),
      title: event.fixClassName,
    };
  });

  if (modEvents?.length > 0) {
    modEvents = modEvents?.filter(
      (event) => moment(event.date).get("date") >= new Date().getDate()
    );
    modEvents = modEvents?.sort(
      (objA, objB) => Number(objA.date) - Number(objB.date)
    );
    modEvents = modEvents.slice(0, 3);
  }
