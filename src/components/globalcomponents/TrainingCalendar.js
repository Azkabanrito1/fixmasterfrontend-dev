...
import ViewTrainingDetails from "../qamastercomponent/dashboardcomponent/training/ViewTrainingDetails";
...
const TrainingCalendar = ({ events }) => {
  ...
  const handleDayClick = (day) => {
    const dayEvents = events?.filter(
      (event) => moment(event?.start).format("YYYY-MM-DD") === day.dateStr
    );
    if (dayEvents.length) {
